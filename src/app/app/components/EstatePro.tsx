...
// Estate Pro — the monetization surface (C-114 §7: listing / premium visibility /
// consultation tier). A real Pi User-to-App payment; this also satisfies the Pi
// Portal "Process a Transaction" step. ADR-007 dual-mode: Hub navigation → Mode 1
// (Hub modal); standalone → Mode 2 (direct createU2APayment). Approves under
// PI_API_KEY_ESTATE (never the default Hub key — C-12 §11).
//
// IMPORTANT (C-114 §4/§6): this is a SERVICE subscription, NOT a property purchase.
// Estate NEVER processes full property value in Pi (legal). Title transfer + capital
// stay with external legal / payment-service / FundX.
...
const MEMO    = 'TEC Estate — Pro (1 month)';
...
export function EstatePro() {
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      setStatus('paying');
      const result = await createU2APayment(PRICE, MEMO, { item_id: ITEM_ID, plan: 'estate_pro' }, internalId);
  ...
  if (status === 'success') {
    return (
      <div style={{ ...card, borderColor: `${TEC_COLORS.success}66` }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: TEC_COLORS.success }}>✅ Estate Pro active</div>
        <div style={{ fontSize: 12, color: TEC_COLORS.subtext, marginTop: 6 }}>
          Payment received. Thanks for supporting TEC Estate 🏡.
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={card}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: TEC_COLORS.gold }}>🏡 Estate Pro</div>
      ...
      <div style={{ fontSize: 12, color: TEC_COLORS.subtext, marginTop: 8, lineHeight: 1.5 }}>
        ...
        dashboard (your properties by type, occupancy, leased/listed, and Zone-verified status),
        premium listing visibility, and priority consultation. A service subscription, not a property purchase:
        Estate handles listings and services — not full property sales.
      </div>
